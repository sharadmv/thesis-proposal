...
\begin{frame}{Challenge: ambiguity}
  \centering
  \onslide<1-5>{Consider the clustering scenario.}
  \onslide<6>{A Bayesian approach models this ambiguity}
...
\begin{frame}{Results}
  ...
  \caption*{Results of hierarchical clustering on MNIST}
  \end{figure}
\end{frame}
...
\begin{frame}{Variational inference at a high level}
  ...
  \pause
    ...
    \end{itemize}
    \pause
    If $q(x)$ is sufficiently expressive,
...
\begin{frame}{Variational autoencoder}
  ...
  \begin{center}
      \ovalbox{Pick $q_\phi(x | y)$ (can be conditional) to be a \emph{neural network} parameterized by $\phi$}
  \end{center}
...
\begin{frame}{Neural variational message passing}
  Neural variational message passing - Vikram 2018
